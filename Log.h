...
constexpr inline bool isTerminal(char c) {
  return c == 'd' || c == 'i' || c == 'u' || c == 'o' || c == 'x' || c == 'X' || c == 'f' || c == 'F' || c == 'e' ||
         c == 'E' || c == 'g' || c == 'G' || c == 'a' || c == 'A' || c == 'c' || c == 'p' || c == '%' || c == 's' ||
         c == 'n';
}
 ...
 */
template<int N>
constexpr inline ParamType getParamInfo(const char (&fmt)[N], int paramNum = 0) {
  int pos = 0;
  while (pos < N - 1) {
    ...
    if (fmt[pos] != '%') {
      ...
      if (fmt[pos] == '%') {
        ...
        if (paramNum != 0) {
          ...
          if (hasDynamicPrecision) return ParamType::STRING_WITH_DYNAMIC_PRECISION;
...
constexpr std::array<ParamType, sizeof...(Indices)> analyzeFormatStringHelper(const char (&fmt)[N],
...
constexpr std::array<ParamType, NParams> analyzeFormatString(const char (&fmt)[N]) {
...
constexpr int getNumNibblesNeeded(const char (&fmt)[N]) {
  ...
  for (int i = 0; i < countFmtParams(fmt); ++i) {
    ...
    if (t == NON_STRING || t == DYNAMIC_PRECISION || t == DYNAMIC_WIDTH) ++numNibbles;
  }
...
inline typename std::enable_if<std::is_same<T, const wchar_t*>::value || std::is_same<T, const char*>::value ||
                                 std::is_same<T, wchar_t*>::value || std::is_same<T, char*>::value,
                               void>::type
store_argument(char** storage, T arg, const ParamType paramType, const size_t stringSize) {
  ...
  if (paramType <= ParamType::NON_STRING) {
    store_argument<const void*>(storage, static_cast<const void*>(arg), paramType, stringSize);
    return;
  }
...
inline void store_arguments(const std::array<ParamType, N>& paramTypes, size_t (&stringBytes)[M], char** storage,
                            T1 head, Ts... rest) {
...
inline void store_arguments(const std::array<ParamType, N>&, size_t (&stringSizes)[M], char**) {
...
template<typename T>
inline
  typename std::enable_if<std::is_convertible<T, uint64_t>::value && !std::is_floating_point<T>::value, uint64_t>::type
  as_uint64_t(T t) {
  return t;
}
...
inline typename std::enable_if<!std::is_same<T, const wchar_t*>::value && !std::is_same<T, const char*>::value &&
                               ...
                               size_t>::type
getArgSize(const ParamType fmtType, uint64_t& previousPrecision, size_t& stringSize, T arg) {
  if (fmtType == ParamType::DYNAMIC_PRECISION) previousPrecision = as_uint64_t(arg);
 ...
 */
inline size_t getArgSize(const ParamType, uint64_t& previousPrecision, size_t& stringSize, const void*) {
  return sizeof(void*);
}
 ...
 */
inline size_t getArgSize(const ParamType fmtType, uint64_t& previousPrecision, size_t& stringBytes, const char* str) {
  ...
  if (fmtType >= ParamType::STRING && stringBytes > fmtLength) stringBytes = fmtLength;
  ...
  else if (fmtType == ParamType::STRING_WITH_DYNAMIC_PRECISION && stringBytes > previousPrecision)
 ...
 */
inline size_t getArgSize(const ParamType fmtType, uint64_t& previousPrecision, size_t& stringBytes,
                         const wchar_t* wstr) {
  if (fmtType <= ParamType::NON_STRING) return sizeof(void*);
 ...
 */
template<int argNum = 0, unsigned long N, int M, typename T1, typename... Ts>
inline size_t getArgSizes(const std::array<ParamType, N>& argFmtTypes, uint64_t& previousPrecision,
                          size_t (&stringSizes)[M], T1 head, Ts... rest) {
  return getArgSize(argFmtTypes[argNum], previousPrecision, stringSizes[argNum], head) +
         getArgSizes<argNum + 1>(argFmtTypes, previousPrecision, stringSizes, rest...);
}
 ...
 */
template<int argNum = 0, unsigned long N, int M>
inline size_t getArgSizes(const std::array<ParamType, N>&, uint64_t&, size_t (&)[M]) {
  return 0;
}
 ...
 */
template<typename T>
inline void compressSingle(TwoNibbles* nibbles, int* nibbleCnt, const ParamType paramType, bool stringsOnly, char** in,
                           char** out) {
  if (paramType > ParamType::NON_STRING) {
...
#pragma GCC diagnostic pop
  ...
  if (*nibbleCnt & 0x1)
    nibbles[*nibbleCnt / 2].second = 0xf & pack(out, argument);
  else
    nibbles[*nibbleCnt / 2].first = 0xf & pack(out, argument);
...
inline __attribute__((__always_inline__)) void compress_internal(TwoNibbles*, int, const bool*, bool, int, char**,
                                                                 char**);
...
inline __attribute__((__always_inline__)) void compressHelper(TwoNibbles* nibbles, int nibbleCnt,
                                                              const ParamType* paramTypes, bool stringsOnly, int argNum,
                                                              char** in, char** out) {
  // Peel off the first argument, and recursively process the rest
  compressSingle<T1>(nibbles, &nibbleCnt, paramTypes[argNum], stringsOnly, in, out);
  compress_internal<Ts...>(nibbles, nibbleCnt, paramTypes, stringsOnly, argNum + 1, in, out);
}

template<typename... Ts>
inline __attribute__((__always_inline__)) void compress_internal(TwoNibbles* nibbles, int nibbleCnt,
                                                                 const ParamType* isArgString, bool stringsOnly,
                                                                 int argNum, char** in, char** out) {
  compressHelper<Ts...>(nibbles, nibbleCnt, isArgString, stringsOnly, argNum, in, out);
}
 ...
 */
template<typename... Ts>
inline void compress(int numNibbles, const ParamType* paramTypes, char** input, char** output) {
  char* in = *input;
  char* out = *output;
  ...
  auto* nibbles = reinterpret_cast<TwoNibbles*>(out);
  out += (numNibbles + 1) / 2;
 ...
 *      format parameters
 */
static void NANOLOG_PRINTF_FORMAT_ATTR(1, 2) checkFormat(const char*, ...) {
}
...
#define logw(format, ...) NANO_LOG(WARNING, format, ##__VA_ARGS__)
#define loge(format, ...) NANO_LOG(ERROR, format, ##__VA_ARGS__)
